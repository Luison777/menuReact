...
type ObjectFit = 'fill' | 'contain' | 'cover' | 'none' | 'scale-down';
        ...
        const imageStyle = {
            ...
            useEffect(() => {
                if(contexto?.state.order.length==0){
       ...
       <Image src="/parednegra.webp" alt="pared" fill={true} style={imageStyle} priority={true}/>
            <ul className="flex h-[93%] overflow-x-auto Lobster relative z-50 rounded">
                {contexto?.state.order.map((section)=>
                <li key={section} className={`${style} `} onClick={()=>{router.push(`/${contexto.state.subsections[section].join('/').replace(/'/g,'')}`)}}>
